...
public abstract class BeanUtils {
	 ...
	 * <code>findDeclaredMethod</code>. This allows to find public methods without
	 ...
	 * <code>findDeclaredMethodWithMinimalParameters</code>. This allows to find
	 ...
	 *             if methods of the given name were found but could not be resolved
	 *             to a unique method with minimal parameters
	 ...
	 * where <code>arg_list</code> is an optional, comma-separated list of
	 * fully-qualified type names, and attempts to resolve that signature against
	 * the supplied <code>Class</code>.
	 * <p>
	 * When not supplying an argument list (<code>methodName</code>) the method
	 * whose name matches and has the least number of parameters will be returned.
	 * When supplying an argument type list, only the method whose name and argument
	 * types match will be returned.
	 * <p>
	 * Note then that <code>methodName</code> and <code>methodName()</code> are
	 * <strong>not</strong> resolved in the same way. The signature
	 * <code>methodName</code> means the method called <code>methodName</code> with
	 * the least number of arguments, whereas <code>methodName()</code> means the
	 * method called <code>methodName</code> with exactly 0 arguments.
	 * <p>
	...
	/**
	 * Find a JavaBeans <code>PropertyDescriptor</code> for the given method, with
	...
	/**
	 * Check if the given class represents a "simple" property: a primitive, a
	 * String, a Class, or a corresponding array.
	 * <p>
	...
	public static boolean isSimpleProperty(Class clazz) {
		Assert.notNull(clazz, "Class must not be null");
		return clazz.isPrimitive() || ClassUtils.isPrimitiveArray(clazz) || ClassUtils.isPrimitiveWrapper(clazz)
				|| ClassUtils.isPrimitiveWrapperArray(clazz) || clazz.equals(String.class)
				|| clazz.equals(String[].class) || clazz.equals(Class.class) || clazz.equals(Class[].class);
	}
	 ...
	 *             <code>ClassUtils.isAssignable</code>
	 ...
	 * @deprecated as of Spring 2.0, in favor of
	 *             <code>ClassUtils.isAssignableValue</code>
